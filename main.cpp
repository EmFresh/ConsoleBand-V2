...
function<bool()>goup = [&]()->bool
{
	...
	if (KeyInput::stroke(VK_UP))return true;


	for (int a = 0; a < 4; a++)
		if (XinputManager::controllerConnected(a))
		{
			if (XinputManager::getController(a)->isButtonStroked(CONTROLLER_INPUT_BUTTONS::CONTROLLER_DPAD_UP))
				return true;
			if (XinputManager::getControllerType(a) == CONTROLLER_TYPE::XINPUT_DRUM)
				if (XinputManager::getController(a)->isButtonStroked(DRUM_INPUT_BUTTONS::DRUM_PAD_YELLOW))
					return true;
		}
	return false;
};
...
{
	if (KeyInput::stroke(VK_DOWN))return true;

	for (int a = 0; a < 4; a++)
		if (XinputManager::controllerConnected(a))
		{
			if (XinputManager::getController(a)->isButtonStroked(CONTROLLER_INPUT_BUTTONS::CONTROLLER_DPAD_DOWN))
				return true;
			if (XinputManager::getControllerType(a) == CONTROLLER_TYPE::XINPUT_DRUM)
				if (XinputManager::getController(a)->isButtonStroked(DRUM_INPUT_BUTTONS::DRUM_PAD_BLUE))
					return true;
		}
	return false;
};
...
SpriteSheet* track = new SpriteSheet, * notes = new SpriteSheet;
Sprite* box = new Sprite, * pauseMenu = new Sprite, * logo = new Sprite;

vector<vector<vector<uint>>>* disappearNotes = new vector<vector<vector<uint>>>;
vector<ushort>
...
void openSong(string songFile)
{
	...
	for (int a = 0; a < (int)trackInfo->size(); a++)
	{
		...
		disappearNotes->push_back(vector<vector<uint>>(5));
	}
			...
			if (trackStr == "PART VOCALS")
			{
				...
				if (file[a][b].isText())
				{
					...
					if (lyricTiming->size())
					{
						...
						if ((file[a][b][3] != '+' && []()->uint
							{
								uint b = 0; for (auto& a : lyrics[0].back())b += (uint)a.size(); return b;
							}() + wstring(file[a][b].begin() + 3, file[a][b].end()).size() + 2u < (uint)lyricBox.getWidth() - 1) &&
				...
				if (file[a][b].isLyricText())
				{
					if (lyricTiming->size())
					{
						...
						if ((file[a][b][3] != '+' && []()->uint
							{
								unsigned b = 0; for (auto& a : lyrics[0].back())b += (uint)a.size(); return b;
							}() + wstring(file[a][b].begin() + 3, file[a][b].end()).size() + 2u < (uint)lyricBox.getWidth() - 1) &&
							(timeInSec - lyricTiming[0].back().back().second < (uint)750))
...
#pragma region Vocal Information 
	...
	uint diff = 120u - (uint)vocalTrack->size();

	//this for loop is illegal but it works
	for (uint a = uint(vocalTrack->size() - 1); a >= 0; a = uint(vocalTrack->size() - 1))
...
void missFX(int index, int seed)
{
	...
	unordered_map<string, vector<string>>miss
	{
...
bool noteDelete(int index)
{
	...
	if (trackInfo[0][index].instrument == "guitar")
	{
		...
		if (num.size() == 1)
		{
			...
			if (pressed[index][num[0]])
				notesHit++,
				(*disappearNotes)[index][num[0]].push_back(colliCountGuitar[num[0]]++);
		}
		else
		{
			int counter = 0, sum = 0;
			...
			for (auto& a : num)
				notesHit++,
				(*disappearNotes)[index][a].push_back(colliCountGuitar[a]++);
		}
	}
	else if (trackInfo[0][index].instrument == "rhythm")
	{
		...
		if (num.size() == 1)
		{
			...
			if (pressed[index][num[0]])
				notesHit++,
				(*disappearNotes)[index][num[0]].push_back(colliCountBass[num[0]]++);
		}
		else
		{
			int counter = 0, sum = 0;
			...
			for (auto& a : num)
				notesHit++,
				(*disappearNotes)[index][a].push_back(colliCountBass[a]++);
		}
	}
			...
			if (colliCountDrum[a] < (*theTrackTmps)[index][a].size())
				if ((*theTrackTmps)[index][a][colliCountDrum[a]].first + noteOffset >= fretboardPosition - 3 && (*theTrackTmps)[index][a][colliCountDrum[a]].first + noteOffset <= fretboardPosition + 3)
					if (initPressed[index][a] && pressed[index][a])
						hitIt = true,
						notesHit++,
						(*disappearNotes)[index][a].push_back(colliCountDrum[a]++);
		if (!hitIt)
			return false;
	}
...
{
	for (uint b = 0; b < (*disappearNotes)[index][fret].size(); b++)
		if ((*disappearNotes)[index][fret][b] == check)
			return true;
...
void playSongMovement()
{
	...
	for (int c = 0; c < (int)theTrackTmps->size(); c++)
	{
		...
		if (trackInfo[0][c].instrument == "guitar")
			...
			{
				for (int b = countAmountGuitar[a]; b < (int)(*theTrackTmps)[c][a].size(); b++)
				{
					...
					if (b > countAmountGuitar[a])
					{
						...
						if (((*theTrackTmps)[c][a][b - 1].first >= (abs((*theTrackTmps)[c][a][b].first) - abs((*theTrackTmps)[c][a][b - (short)1].first))))//if the note below is off screen? (actually idk)
							if ((*disappearNotes)[c][a].size() > 0)
								if ((uint)b == (*disappearNotes)[c][a][0])
									(*disappearNotes)[c][a].erase((*disappearNotes)[c][a].begin());
					...
					if (!invisable(c, a, b))
						if ((*theTrackTmps)[c][a][b].first + noteOffset > -3 && (*theTrackTmps)[c][a][b].first + noteOffset < EmConsole::getHeight())
						{
							...
							if ((*theTrackTmps)[c][a][b].first + noteOffset < fretboardPosition + notes[0][0].getHeight())//draw notes above the fret board
							{
								...
								///Collision Test
								EmConsole::toConsoleBuffer((*notes)[0], (centerTrack)+(a * 13 + 2), (*theTrackTmps)[c][a][b].first + noteOffset,
									b != (int)colliCountGuitar[a] ? (*fretNoteColour)[a] : FG_INTENSIFY | FG_WHITE);
	...
	for (uint a = firstLyric; a < lyrics->size(); a++)
	{
		...
		if (incriment > (long)lyricTiming[0][a].front().first)
		{
			...
			for (uint b = 1; b < lyrics[0][a].size(); b++)
				...
				EmConsole::toConsoleBuffer(lyrics[0][a][b], int(EmConsole::getWidth() / 2 - phrase.size() / 2 + (length += (uint)lyrics[0][a][b - 1].size())), 2,
	...
	if (!paused)
	{
		...
		for (int b = 0; b < (int)theTracks->size(); b++)
		{
			if (trackInfo[0][b].instrument == "guitar" || trackInfo[0][b].instrument == "rhythm")
			{
				...
				if ([&]()->bool
					{
						up = XinputManager::getController(trackInfo[0][b].player)->
							isButtonPressed(GUITAR_STRUM_UP);

							down = XinputManager::getController(trackInfo[0][b].player)->
							isButtonPressed(GUITAR_STRUM_DOWN);
...
void playTrack()
{
	...
	*t1 = thread(percent);
	...
	EmConsole::toConsoleBuffer(*percentstr, int(EmConsole::getWidth() - (*box).getWidth() / 2 - (percentstr->size() + 1) / 2), 2);
...
void songChoiceMovement(int size)
{
	...
	for (int a = 0; a < 4; a++)
		if (!goup() && !godown())
		{
			lastClockT[a] = clock() - uint(CLOCKS_PER_SEC * delay);
			break;
		}
}
...
bool songList()
{
	static wstring path = L"E:/Phase Shift/music";
	...
	while (true)
	{
		...
		if (pass || (MouseInput::stroke(LEFT_CLICK) &&
			Sprite(vector<wstring> {songs[songChoice - movement]}).
			mouseCollision({ float(startWidth - (songs[songChoice - movement].size() / 2)),
				float(startHeight + songChoice - movement) }, MouseInput::position)))
		{
		...
		if (clock >= lastClock + (sample * 25))
		{
			...
			AudioPlayer::getMasterChannelGroup()->addFadePoint(clock, 0);
			AudioPlayer::getMasterChannelGroup()->addFadePoint(clock + (sample * 2), 1);//full vol at 2sec
			AudioPlayer::getMasterChannelGroup()->addFadePoint(clock + (sample * 23), 1);//start to fade out at 23sec
			AudioPlayer::getMasterChannelGroup()->addFadePoint(clock + (sample * 25), 0);//no vol at 25sec
			lastClock = clock;
		}

		if (count - (endHeight - startHeight) < 0)//
			movement = 0;
		else if (songChoice >= (endHeight - startHeight) / 3 && songChoice < count - (endHeight - startHeight) / 3)
			movement =
			(
		...
		for (uint a = 0; a < songs.size(); a++)
			EmConsole::toConsoleBuffer(songs[a], int(startWidth - (songs[a].size() / 2)), startHeight + a, ((uint)songChoice - movement == a ? colours[1] : colours[0]));
		...
		for (int a = 0; a < 2; a++)
			...
			EmConsole::toConsoleBuffer(noteMsg[a], int(EmConsole::getWidth() * .1f) + 12 * a + notes[0][0].getWidth() / 2 - (int)noteMsg[a].size() / 2, EmConsole::getHeight() - 3, fretNoteColour[0][a]),
			EmConsole::toConsoleBuffer(noteKey[a], int(EmConsole::getWidth() * .1f) + 12 * a + notes[0][0].getWidth() / 2 - (int)noteKey[a].size() / 2, EmConsole::getHeight() - 5, fretNoteColour[0][a]);
...
bool difficultyMenu()
{
	...
	while (true)
	{
		...
		for (int b = 0; b < (int)trackInfo->size(); b++)
		{
			...
			int spacing = 35 / 4 * ((int)trackInfo->size() - 1 < 0 ? 0 : (int)trackInfo->size() - 1);

			EmConsole::toConsoleBuffer(L"Difficulty", EmConsole::getWidth() / 2 - 5 - (18 * ((int)trackInfo->size() - b - 1)) + spacing, y - 3);
			EmConsole::toConsoleBuffer(wstring(instruments[selectx[trackInfo[0][b].player]].begin(), instruments[selectx[trackInfo[0][b].player]].end()).c_str(),
				EmConsole::getWidth() / 2 - int(instruments[selectx[trackInfo[0][b].player]].size()) / 2 - (18 * (int(trackInfo->size()) - b - 1)) + spacing, y - 1, FG_RED | FG_INTENSIFY);
			EmConsole::toConsoleBuffer(L"-----------------------------------", EmConsole::getWidth() / 2 - 18 - (18 * (int(trackInfo->size()) - b - 1)) + spacing, y);

			if (!ready[trackInfo[0][b].player])
				//draw selection boxes
				for (int a = 0; a < numBoxes; a++)
					EmConsole::toConsoleBuffer(*box, x - (18 * ((int)trackInfo->size() - b - 1)) + spacing, y + a * 5 + 1, colours[selecty[trackInfo[0][b].player] == a]),
					EmConsole::toConsoleBuffer(options[a], x + (box->getWidth() / 2) - int(options[a].size()) / 2 - (18 * (int(trackInfo->size()) - b - 1)) + spacing, y + a * 5 + 3, colours[selecty[trackInfo[0][b].player] == a]);
			else
				for (int a = 0; a < numBoxes; a++)
					EmConsole::toConsoleBuffer(*box, x - (18 * ((int)trackInfo->size() - b - 1)) + spacing, y + a * 5 + 1, colours[1]),
					EmConsole::toConsoleBuffer(L"READY", x + (box->getWidth() / 2) - 5 / 2 - (18 * ((int)trackInfo->size() - b - 1)) + spacing, y + a * 5 + 3, colours[1]);
...
void controls()
{
	...
	controls.create("Game Files/Controles.txt");
	while ((XinputManager::update(), !selector() && !deselect()))
	{
		EmConsole::toConsoleBuffer(controls, EmConsole::getWidth() / 2 - controls.getWidth() / 2, EmConsole::getHeight() / 2 - controls.getHeight() / 2);
		int a = 0;
		EmConsole::toConsoleBuffer(notes[0][0], int(EmConsole::getWidth() * .1f) + 12 * a, EmConsole::getHeight() - 4, fretNoteColour[0][a + 1]);
		EmConsole::toConsoleBuffer(noteMsg[a], int(EmConsole::getWidth() * .1f) + 12 * a + notes[0][0].getWidth() / 2 - (int)noteMsg[a].size() / 2, EmConsole::getHeight() - 3, fretNoteColour[0][a + 1]);
		EmConsole::toConsoleBuffer(noteKey[a], int(EmConsole::getWidth() * .1f) + 12 * a + notes[0][0].getWidth() / 2 - (int)noteKey[a].size() / 2, EmConsole::getHeight() - 5, fretNoteColour[0][a + 1]);
...
bool startScreen()
{
	...
	while (!exit)
	{
		...
		EmConsole::toConsoleBuffer(*logo, EmConsole::getWidth() / 2 - logo->getWidth() / 2, 10, FG_RED | FG_INTENSIFY);


		//draw selection boxes
		for (int a = 0; a < numBoxes; a++)
			EmConsole::toConsoleBuffer(*box, x, y + a * 5, colours[create == a]),
			EmConsole::toConsoleBuffer(options[a], x + (box->getWidth() / 2) - (int)options[a].size() / 2, y + a * 5 + 2, colours[create == a]);

		for (int a = 0; a < 1; a++)
			EmConsole::toConsoleBuffer(notes[0][0], int(EmConsole::getWidth() * .1f) + 10 * a, EmConsole::getHeight() - 4, fretNoteColour[0][a]),
			EmConsole::toConsoleBuffer(noteMsg[a], int(EmConsole::getWidth() * .1f) + 10 * a + notes[0][0].getWidth() / 2 - int(noteMsg[a].size()) / 2, EmConsole::getHeight() - 3, fretNoteColour[0][a]),
			EmConsole::toConsoleBuffer(noteKey[a], int(EmConsole::getWidth() * .1f) + 10 * a + notes[0][0].getWidth() / 2 - int(noteKey[a].size()) / 2, EmConsole::getHeight() - 5, fretNoteColour[0][a]);
